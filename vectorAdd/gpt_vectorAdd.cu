...
void reduce_sum(const float *input_vecs, size_t n, size_t dim, float *output_vec) {
	...
	checkCudaError(cudaMemcpy(cuda_input_vecs, input_vecs, n * dim * sizeof(float), cudaMemcpyHostToDevice),
				   "cudaMemcpy input_vecs to cuda_input_vecs");

	// Copy the initialized output vector from host to device
	checkCudaError(cudaMemcpy(cuda_output_vec, output_vec, dim * sizeof(float), cudaMemcpyHostToDevice),
	...
	checkCudaError(cudaMemcpy(output_vec, cuda_output_vec, dim * sizeof(float), cudaMemcpyDeviceToHost),
